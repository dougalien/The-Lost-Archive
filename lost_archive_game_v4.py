...
def build_tiles(seed: int, topic: str, pack: List[Dict]) -> Dict[Tuple[int, int], Dict]:
    ...
    names = ROOM_NAMES.get(topic, ROOM_NAMES["Custom"])
    names = names[:]
    rng.shuffle(names)
        ...
        for x in range(SIZE):
            ...
            if pos == START:
                tiles[pos] = {
                    "kind": "start",
                    "name": "Entry Foyer",
                    "desc": "The first lamp is already lit. A quiet map waits for you to earn your next steps.",
                    "question": None,
                    "visited": True,
                    "event": None,
                    "event_used": True,
            ...
            if pos == VAULT:
                tiles[pos] = {
                    "kind": "vault",
                    "name": "Heart Vault",
                    "desc": "The final chamber is present but dormant. Enough Archive Seals will wake it.",
                    "question": None,
                    "visited": False,
                    "event": None,
                    "event_used": True,
            ...
            if event_type == "lumen_block":
                ...
                desc = rng.choice(ROOM_DESCRIPTORS)
...
def maybe_add_story_milestone() -> None:
    ...
    milestones = TOPIC_MILESTONES[topic]
...
def update_vault_ready() -> None:
    if not st.session_state.vault_ready and st.session_state.seals >= st.session_state.seals_needed:
        ...
        add_log("A strong tone rolls through the archive. The Heart Vault is now ready to open.")
...
def apply_tile_event(pos: Tuple[int, int]) -> None:
    ...
    if event_type == "lumen_block":
        ...
        add_log("You found the Lumen Block. Gain +3 Insight and +1 safeguard.")
    elif event_type == "token_cache":
        gain = 2
        st.session_state.tokens += gain
        add_log(f"You found a pile of tokens. Gain +{gain} Insight.")
    elif event_type == "secret_passage":
        st.session_state.free_passes += 1
        add_log("A secret passage clicks into place. Your next unrevealed room opens for free and still earns a Seal.")
    elif event_type == "seal_fragment":
        if st.session_state.seals < st.session_state.seals_needed:
            st.session_state.seals += 1
            add_log("You recover a Seal Fragment. It counts as +1 Archive Seal.")
        ...
        if st.session_state.get("accessible_play_mode"):
            st.session_state.tokens += 1
            add_log("The archive tries to dim the controls, but accessible play mode keeps the labels visible. You get +1 Insight instead.")
        else:
            st.session_state.darkness_turns = 1
            add_log("All the lights go out. On your next move, the direction buttons will lose their room labels.")
...
def inspect_room_text() -> str:
    tile = current_tile()
    extras = {
        "start": "The foyer map suggests many routes, but only earned answers will stabilize new rooms.",
        "standard": "A small label suggests the archive improves through repetition, clarity, and steady effort.",
        "vault": "The Heart Vault is quiet for now. When enough Seals are earned, it will be ready from anywhere in the archive.",
    }
...
def choose_direction(direction: str) -> None:
    ...
    if st.session_state.free_passes > 0:
        ...
        add_log(f"A secret route opens. You enter {tile['name']} without a question and still earn 1 Archive Seal.")
        maybe_add_story_milestone()
        update_vault_ready()
    ...
    if next_question is None:
        st.session_state.feedback = (
            "You have used every unique question in this run. Restart the run or use a larger pack for no-repeat play."
        )
        return
...
def reset_game(pack: Optional[List[Dict]] = None, pack_name: Optional[str] = None, pack_source: str = "built-in") -> None:
    ...
    tiles = build_tiles(seed, topic if pack_source == "built-in" else "Custom", pack)
    intro_key = topic if pack_source == "built-in" else "Custom"
    ...
    st.session_state.story_log = TOPIC_INTRO[intro_key][:]
...
def render_start() -> None:
    ...
    st.info("Current cost mode: no required ChatGPT calls. This build uses built-in or uploaded question packs only. Add OPENAI_API_KEY later if you want optional AI features.")
    st.markdown(
...
def render_header() -> None:
    ...
    c1.metric("Seals", f"{st.session_state.seals}/{st.session_state.seals_needed}")
    c2.metric("Insight", st.session_state.tokens)
    ...
    st.markdown(
        f"<div class='small'>Pack: {st.session_state.pack_name} · Difficulty: {st.session_state.difficulty} · "
        f"Questions left: {len(st.session_state.question_deck)} · Heart Vault: {'ready' if st.session_state.vault_ready else 'locked'}</div>",
        unsafe_allow_html=True,
    )
    ...
    with c2:
        if st.button("Archive notes"):
            if st.session_state.vault_ready:
                add_log("The Heart Vault is awake. You can finish the run whenever you want.")
            else:
                left = max(0, st.session_state.seals_needed - st.session_state.seals)
                add_log(f"The archive estimates {left} more Seal{'s' if left != 1 else ''} are needed to wake the Heart Vault.")
            st.rerun()
    with c3:
        if st.button("Open Heart Vault"):
            if st.session_state.vault_ready:
                ...
                st.info("The Heart Vault is still locked. Earn more Seals first.")
    with c4:
...
def finish_run() -> None:
    ...
    add_log("The Heart Vault opens. Shelves, labels, and pathways reconnect across the archive.")
...
def render_question_card() -> None:
    ...
    st.markdown(
        ...
        f"<div class='soft'>Answer correctly to move into this room and earn 1 Archive Seal.</div></div>",
        unsafe_allow_html=True,
    )
...
def render_end() -> None:
    ...
    st.success(f"{name}, you restored the archive.")
    st.write(
        "This run used a fixed Seal goal, while the map supplied the story, random finds, and a few surprises. "
        "That makes the engine portable across topics and more useful as a repeatable class tool."
    )
...
def main() -> None:
    ...
    st.title("The Lost Archive")
    render_brand_banner()
    st.caption("Earn Seals, discover side rewards, and finish with a clear learning summary.")
    render_accessibility_controls()
    ...
    with st.expander("Run notes"):
        ...
        st.write("Questions are drawn from a shuffled no-repeat deck for each run. Use the upload option to turn the same engine into a review game for almost any discipline.")
        render_feedback_panel()
